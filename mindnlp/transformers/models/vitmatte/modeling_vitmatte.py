...
class VitMatteFusionBlock(nn.Cell):
    ...
    def construct(self, features, detailed_feature_map):
        upscaled_features = ops.interpolate(features, scale_factor=2, mode="bilinear", align_corners=False)
        out = ops.cat([detailed_feature_map, upscaled_features], axis=1)
        out = self.conv(out)
...
class VitMatteForImageMatting(VitMattePreTrainedModel):
    ...
    def construct(
        ...
        """
        labels (`mindspore.LongTensor` of shape `(batch_size, height, width)`, *optional*):
