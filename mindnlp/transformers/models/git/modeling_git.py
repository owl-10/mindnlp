...
class GitSelfAttention(nn.Cell):
    ...
    def construct(
        ...
        if attention_mask is not None:
            # Apply the attention mask is (precomputed for all layers in GitModel construct() function)
...
class GitVisionEmbeddings(nn.Cell):
    def __init__(self, config: GitVisionConfig):
        ...
        self.position_ids = ops.arange(self.num_positions).reshape((1, -1))
...
class GitModel(GitPreTrainedModel):
    def __init__(self, config):
        ...
        if config.num_image_with_embedding is not None:
            self.img_temperal_embedding.append(
                Parameter(ops.zeros((1, 1, config.vision_config.hidden_size)))
                for _ in range(config.num_image_with_embedding)
            )
        self.img_temperal_embedding  = ParameterTuple(self.img_temperal_embedding )
        # Initialize weights and apply final processing
        self.post_init()
